...
class VideoDataset_images_with_motion_features(data.Dataset):
    ...
    def __init__(self, data_dir, data_dir_3D, filename_path_list, transform, database_name, crop_size, feature_type, mode):
        super(VideoDataset_images_with_motion_features, self).__init__()

        # if database_name == 'VEC_train':
        # dataInfo = scio.loadmat(filename_path)
        # n = len(dataInfo['video_names'])
        dataInfo = pd.read_csv(filename_path_list, header=None, sep=',', index_col=False)
        n = len(dataInfo)
        video_names = []
        score = []
        # index_all = dataInfo['index'][0]
        for i in range(n):
            video_names.append(list(dataInfo[0])[i])
            score.append(list(dataInfo[1])[i])
    ...
    def __getitem__(self, idx):
        # if self.database_name == 'VEC_train' or self.database_name == 'VEC_valid':
        #     video_name = self.video_names[idx]
        #     video_name_str = video_name[:-4]
        # elif self.database_name == 'VEC_test':
        ...
        # if self.database_name == 'VEC_train' or self.database_name == 'VEC_train' or self.database_name == 'LSVQ_test' \
        #         or self.database_name == 'LSVQ_test_1080p':
        #     video_length_read = 8
        # elif self.database_name == 'youtube_ugc':

        list_name = os.listdir(path_name)
        # print(len(list_name))
        video_length_read = 8 # len(list_name)

        if self.mode =='train':
            transformed_video = torch.zeros([video_length_read, video_channel, video_height_crop, video_width_crop])
        else:
            transformed_video = torch.zeros([video_length_read, 10, video_channel, video_height_crop, video_width_crop])

        for i in range(video_length_read):
            image_name = os.path.join(path_name, list_name[i])
            # print(image_name)
        ...
        if self.feature_type == 'Slow':
            ...
            feature_folder_name = os.path.join(self.data_dir_3D, video_name_str+'.mp4')
            feature_num = 8 # len(os.listdir(feature_folder_name)) // 2
...
class VideoDataset_images_with_motion_features_test(data.Dataset):
    """Read data from the original dataset for feature extraction"""

    def __init__(self, data_dir, data_dir_3D, filename_path_list, transform, database_name, crop_size, feature_type):
        super(VideoDataset_images_with_motion_features_test, self).__init__()

        # if database_name == 'VEC_valid':
        # dataInfo = scio.loadmat(filename_path)
        n = len(filename_path_list)
        video_names = []
            # score = []
            # index_all = dataInfo['index'][0]
        for i in range(n):
            video_names.append(filename_path_list[i])
...
class VideoDataset_NR_VEC_SlowFast_feature(data.Dataset):
    """Read data from the original dataset for feature extraction"""

    def __init__(self, data_dir, filename_path, transform, resize, is_test_1080p=False):
        super(VideoDataset_NR_VEC_SlowFast_feature, self).__init__()
        column_names = ['name', 'p1', 'p2', 'p3', \
                        'height', 'width', 'mos_p1', \
                        'mos_p2', 'mos_p3', 'mos', \
                        'frame_number', 'fn_last_frame', 'left_p1', \
        ...
        #                        encoding="utf-8-sig")
        dataInfo = pd.read_csv(filename_path, header=None, sep=',', index_col=False,
                               encoding="utf-8-sig")

        self.video_names = dataInfo[0]
        self.score = dataInfo[1]
        self.videos_dir = data_dir
        self.transform = transform
        self.resize = resize
        self.length = len(self.video_names)
    ...
    def __getitem__(self, idx):
        video_name = list(self.video_names)[idx]
        video_score = torch.FloatTensor(np.array(float(self.score.iloc[idx]))) / 20
...
class VideoDataset_NR_VEC_TEST_SlowFast_feature(data.Dataset):
    """Read data from the original dataset for feature extraction"""

    def __init__(self, data_dir, filename_list, transform, resize, is_test_1080p=False):
        super(VideoDataset_NR_VEC_TEST_SlowFast_feature, self).__init__()
        column_names = ['name', 'p1', 'p2', 'p3', \
                        'height', 'width', 'mos_p1', \
                        'mos_p2', 'mos_p3', 'mos', \
                        'frame_number', 'fn_last_frame', 'left_p1', \
        ...
        self.video_names = filename_list
        # self.score = dataInfo[1]
        self.videos_dir = data_dir
        self.transform = transform
        self.resize = resize
